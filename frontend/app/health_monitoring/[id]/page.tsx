...
export default function HealthMonitoringDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["health_monitoring", id],
    queryFn: () => api.get<HealthMonitoring>(`/api/health_monitorings/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/health_monitorings/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["health_monitoring"] })
      toast.success("HealthMonitoring deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">HealthMonitoring not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">HealthMonitoring</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/health_monitoring/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete HealthMonitoring?</DialogTitle>
