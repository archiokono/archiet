...
export default function MessageBrokerDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["message_broker", id],
    queryFn: () => api.get<MessageBroker>(`/api/message_brokers/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/message_brokers/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["message_broker"] })
      toast.success("MessageBroker deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">MessageBroker not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">MessageBroker</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/message_broker/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete MessageBroker?</DialogTitle>
