...
export default function KeyMetricsAndKpIsDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["key_metrics_and_kp_is", id],
    queryFn: () => api.get<KeyMetricsAndKpIs>(`/api/key_metrics_and_kp_es/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/key_metrics_and_kp_es/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["key_metrics_and_kp_is"] })
      toast.success("KeyMetricsAndKpIs deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">KeyMetricsAndKpIs not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">KeyMetricsAndKpIs</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/key_metrics_and_kp_is/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete KeyMetricsAndKpIs?</DialogTitle>
