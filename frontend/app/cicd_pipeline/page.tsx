...
export default function CicdPipelineListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["cicd_pipeline", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: CicdPipeline[]; total: number } | CicdPipeline[]>("/api/cicd_pipelines", {
  ...
  const items: CicdPipeline[] = Array.isArray(data) ? data : ((data as { items: CicdPipeline[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">CicdPipelines</h1>
          <p className="text-sm text-muted-foreground">
            Manage cicdpipeline records
          </p>
        </div>
        <Link href="/cicd_pipeline/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search cicdpipelines..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No cicdpipelines yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first cicdpipeline to get started.</p>
              <Link href="/cicd_pipeline/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/cicd_pipeline/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/cicd_pipeline/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/cicd_pipeline/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
