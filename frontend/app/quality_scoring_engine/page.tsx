...
export default function QualityScoringEngineListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["quality_scoring_engine", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: QualityScoringEngine[]; total: number } | QualityScoringEngine[]>("/api/quality_scoring_engines", {
  ...
  const items: QualityScoringEngine[] = Array.isArray(data) ? data : ((data as { items: QualityScoringEngine[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">QualityScoringEngines</h1>
          <p className="text-sm text-muted-foreground">
            Manage qualityscoringengine records
          </p>
        </div>
        <Link href="/quality_scoring_engine/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search qualityscoringengines..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No qualityscoringengines yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first qualityscoringengine to get started.</p>
              <Link href="/quality_scoring_engine/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create QualityScoringEngine
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/quality_scoring_engine/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/quality_scoring_engine/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/quality_scoring_engine/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
