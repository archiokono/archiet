...
export default function QualityScoringEngineDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["quality_scoring_engine", id],
    queryFn: () => api.get<QualityScoringEngine>(`/api/quality_scoring_engines/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/quality_scoring_engines/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["quality_scoring_engine"] })
      toast.success("QualityScoringEngine deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">QualityScoringEngine not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">QualityScoringEngine</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/quality_scoring_engine/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete QualityScoringEngine?</DialogTitle>
