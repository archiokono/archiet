...
export default function GenomeSpecDefinitionsDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["genome_spec_definitions", id],
    queryFn: () => api.get<GenomeSpecDefinitions>(`/api/genome_spec_definitions/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/genome_spec_definitions/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["genome_spec_definitions"] })
      toast.success("GenomeSpecDefinitions deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GenomeSpecDefinitions not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GenomeSpecDefinitions</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/genome_spec_definitions/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GenomeSpecDefinitions?</DialogTitle>
