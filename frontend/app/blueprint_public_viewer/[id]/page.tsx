...
export default function BlueprintPublicViewerDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["blueprint_public_viewer", id],
    queryFn: () => api.get<BlueprintPublicViewer>(`/api/blueprint_public_viewers/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/blueprint_public_viewers/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["blueprint_public_viewer"] })
      toast.success("BlueprintPublicViewer deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">BlueprintPublicViewer not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">BlueprintPublicViewer</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/blueprint_public_viewer/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete BlueprintPublicViewer?</DialogTitle>
