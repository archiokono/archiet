...
export default function BusinessLogicLayerListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["business_logic_layer", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: BusinessLogicLayer[]; total: number } | BusinessLogicLayer[]>("/api/business_logic_layers", {
  ...
  const items: BusinessLogicLayer[] = Array.isArray(data) ? data : ((data as { items: BusinessLogicLayer[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">BusinessLogicLayers</h1>
          <p className="text-sm text-muted-foreground">
            Manage businesslogiclayer records
          </p>
        </div>
        <Link href="/business_logic_layer/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search businesslogiclayers..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No businesslogiclayers yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first businesslogiclayer to get started.</p>
              <Link href="/business_logic_layer/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create BusinessLogicLayer
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/business_logic_layer/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/business_logic_layer/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/business_logic_layer/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
