...
export default function GdprComplianceServiceListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["gdpr_compliance_service", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: GdprComplianceService[]; total: number } | GdprComplianceService[]>("/api/gdpr_compliance_services", {
  ...
  const items: GdprComplianceService[] = Array.isArray(data) ? data : ((data as { items: GdprComplianceService[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">GdprComplianceServices</h1>
          <p className="text-sm text-muted-foreground">
            Manage gdprcomplianceservice records
          </p>
        </div>
        <Link href="/gdpr_compliance_service/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search gdprcomplianceservices..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No gdprcomplianceservices yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first gdprcomplianceservice to get started.</p>
              <Link href="/gdpr_compliance_service/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/gdpr_compliance_service/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/gdpr_compliance_service/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/gdpr_compliance_service/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
