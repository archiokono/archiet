...
export default function CicdToolchainListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["cicd_toolchain", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: CicdToolchain[]; total: number } | CicdToolchain[]>("/api/cicd_toolchains", {
  ...
  const items: CicdToolchain[] = Array.isArray(data) ? data : ((data as { items: CicdToolchain[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">CicdToolchains</h1>
          <p className="text-sm text-muted-foreground">
            Manage cicdtoolchain records
          </p>
        </div>
        <Link href="/cicd_toolchain/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search cicdtoolchains..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No cicdtoolchains yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first cicdtoolchain to get started.</p>
              <Link href="/cicd_toolchain/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create CicdToolchain
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/cicd_toolchain/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/cicd_toolchain/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/cicd_toolchain/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
