...
export default function SmtpRelayGcpListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["smtp_relay_gcp", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: SmtpRelayGcp[]; total: number } | SmtpRelayGcp[]>("/api/smtp_relay_gcps", {
  ...
  const items: SmtpRelayGcp[] = Array.isArray(data) ? data : ((data as { items: SmtpRelayGcp[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">SmtpRelayGcps</h1>
          <p className="text-sm text-muted-foreground">
            Manage smtprelaygcp records
          </p>
        </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search smtprelaygcps..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No smtprelaygcps yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first smtprelaygcp to get started.</p>
              <Link href="/smtp_relay_gcp/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/smtp_relay_gcp/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/smtp_relay_gcp/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/smtp_relay_gcp/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
