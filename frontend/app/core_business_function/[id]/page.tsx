...
export default function CoreBusinessFunctionDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["core_business_function", id],
    queryFn: () => api.get<CoreBusinessFunction>(`/api/core_business_functions/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/core_business_functions/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core_business_function"] })
      toast.success("CoreBusinessFunction deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">CoreBusinessFunction not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">CoreBusinessFunction</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/core_business_function/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete CoreBusinessFunction?</DialogTitle>
