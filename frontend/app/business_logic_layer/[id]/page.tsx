...
export default function BusinessLogicLayerDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["business_logic_layer", id],
    queryFn: () => api.get<BusinessLogicLayer>(`/api/business_logic_layers/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/business_logic_layers/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["business_logic_layer"] })
      toast.success("BusinessLogicLayer deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">BusinessLogicLayer not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">BusinessLogicLayer</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/business_logic_layer/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete BusinessLogicLayer?</DialogTitle>
