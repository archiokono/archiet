...
export default function AnalyticsEngineListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["analytics_engine", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: AnalyticsEngine[]; total: number } | AnalyticsEngine[]>("/api/analytics_engines", {
  ...
  const items: AnalyticsEngine[] = Array.isArray(data) ? data : ((data as { items: AnalyticsEngine[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">AnalyticsEngines</h1>
          <p className="text-sm text-muted-foreground">
            Manage analyticsengine records
          </p>
        </div>
        <Link href="/analytics_engine/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search analyticsengines..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No analyticsengines yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first analyticsengine to get started.</p>
              <Link href="/analytics_engine/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/analytics_engine/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/analytics_engine/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/analytics_engine/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
