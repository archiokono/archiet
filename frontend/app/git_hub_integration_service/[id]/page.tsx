...
export default function GitHubIntegrationServiceDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["git_hub_integration_service", id],
    queryFn: () => api.get<GitHubIntegrationService>(`/api/git_hub_integration_services/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/git_hub_integration_services/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["git_hub_integration_service"] })
      toast.success("GitHubIntegrationService deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GitHubIntegrationService not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GitHubIntegrationService</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/git_hub_integration_service/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GitHubIntegrationService?</DialogTitle>
      ...
      <Card>
        ...
        <CardContent>
          <dl className="divide-y">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Endpoint Url</dt>
              <dd className="flex-1 text-sm">

                {item.endpoint_url ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Service Type</dt>
              <dd className="flex-1 text-sm">

                {item.service_type ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Version</dt>
              <dd className="flex-1 text-sm">
