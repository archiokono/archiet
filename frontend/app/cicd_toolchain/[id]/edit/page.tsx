...
export default function CicdToolchainEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/cicd_toolchains/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/cicd_toolchains/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cicd_toolchain", id] })
      toast.success("CicdToolchain updated")
      router.push(`/cicd_toolchain/${id}`)
    },
    onError: (err: unknown) => {
