...
export default function AnalyticsEngineDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["analytics_engine", id],
    queryFn: () => api.get<AnalyticsEngine>(`/api/analytics_engines/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/analytics_engines/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["analytics_engine"] })
      toast.success("AnalyticsEngine deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">AnalyticsEngine not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">AnalyticsEngine</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/analytics_engine/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete AnalyticsEngine?</DialogTitle>
