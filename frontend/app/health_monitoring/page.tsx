...
export default function HealthMonitoringListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["health_monitoring", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: HealthMonitoring[]; total: number } | HealthMonitoring[]>("/api/health_monitorings", {
  ...
  const items: HealthMonitoring[] = Array.isArray(data) ? data : ((data as { items: HealthMonitoring[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">HealthMonitorings</h1>
          <p className="text-sm text-muted-foreground">
            Manage healthmonitoring records
          </p>
        </div>
        <Link href="/health_monitoring/new">
          <Button>
            <Plus className="mr-2 h-4 w-4" />
            New HealthMonitoring
          </Button>
        </Link>
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search healthmonitorings..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No healthmonitorings yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first healthmonitoring to get started.</p>
              <Link href="/health_monitoring/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create HealthMonitoring
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/health_monitoring/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/health_monitoring/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/health_monitoring/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
