...
export default function CoreBusinessEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/core_businesses/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/core_businesses/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core_business", id] })
      toast.success("CoreBusiness updated")
      router.push(`/core_business/${id}`)
    },
    onError: (err: unknown) => {
