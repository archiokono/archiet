...
export default function VeritasNetbackupSaaSDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["veritas_netbackup_saa_s", id],
    queryFn: () => api.get<VeritasNetbackupSaaS>(`/api/veritas_netbackup_saa_s/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/veritas_netbackup_saa_s/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["veritas_netbackup_saa_s"] })
      toast.success("VeritasNetbackupSaaS deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">VeritasNetbackupSaaS not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">VeritasNetbackupSaaS</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/veritas_netbackup_saa_s/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete VeritasNetbackupSaaS?</DialogTitle>
