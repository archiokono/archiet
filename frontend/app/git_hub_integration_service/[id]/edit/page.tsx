...
export default function GitHubIntegrationServiceEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/git_hub_integration_services/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/git_hub_integration_services/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["git_hub_integration_service", id] })
      toast.success("GitHubIntegrationService updated")
      router.push(`/git_hub_integration_service/${id}`)
    },
    onError: (err: unknown) => {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/git_hub_integration_service/${id}`}>
          <Button variant="ghost" size="icon">
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <h1 className="text-2xl font-semibold tracking-tight">
          Edit GitHubIntegrationService
        </h1>
      </div>
