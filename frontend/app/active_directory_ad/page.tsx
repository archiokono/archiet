...
export default function ActiveDirectoryAdListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["active_directory_ad", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: ActiveDirectoryAd[]; total: number } | ActiveDirectoryAd[]>("/api/active_directory_ads", {
  ...
  const items: ActiveDirectoryAd[] = Array.isArray(data) ? data : ((data as { items: ActiveDirectoryAd[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">ActiveDirectoryAds</h1>
          <p className="text-sm text-muted-foreground">
            Manage activedirectoryad records
          </p>
        </div>
        <Link href="/active_directory_ad/new">
          <Button>
            <Plus className="mr-2 h-4 w-4" />
            New ActiveDirectoryAd
          </Button>
        </Link>
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search activedirectoryads..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No activedirectoryads yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first activedirectoryad to get started.</p>
              <Link href="/active_directory_ad/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create ActiveDirectoryAd
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/active_directory_ad/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/active_directory_ad/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/active_directory_ad/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
