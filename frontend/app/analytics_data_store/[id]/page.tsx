...
export default function AnalyticsDataStoreDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["analytics_data_store", id],
    queryFn: () => api.get<AnalyticsDataStore>(`/api/analytics_data_stores/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/analytics_data_stores/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["analytics_data_store"] })
      toast.success("AnalyticsDataStore deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">AnalyticsDataStore not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">AnalyticsDataStore</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/analytics_data_store/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete AnalyticsDataStore?</DialogTitle>
      ...
      <Card>
        ...
        <CardContent>
          <dl className="divide-y">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Metric Name</dt>
              <dd className="flex-1 text-sm">

                {item.metric_name ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Value</dt>
              <dd className="flex-1 text-sm">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Target</dt>
              <dd className="flex-1 text-sm">

                {item.target ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Unit</dt>
              <dd className="flex-1 text-sm">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Measured At</dt>
              <dd className="flex-1 text-sm">
