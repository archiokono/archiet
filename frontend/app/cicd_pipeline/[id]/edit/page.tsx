...
export default function CicdPipelineEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/cicd_pipelines/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/cicd_pipelines/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cicd_pipeline", id] })
      toast.success("CicdPipeline updated")
      router.push(`/cicd_pipeline/${id}`)
    },
    onError: (err: unknown) => {
