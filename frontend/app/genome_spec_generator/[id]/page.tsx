...
export default function GenomeSpecGeneratorDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["genome_spec_generator", id],
    queryFn: () => api.get<GenomeSpecGenerator>(`/api/genome_spec_generators/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/genome_spec_generators/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["genome_spec_generator"] })
      toast.success("GenomeSpecGenerator deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GenomeSpecGenerator not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GenomeSpecGenerator</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/genome_spec_generator/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GenomeSpecGenerator?</DialogTitle>
