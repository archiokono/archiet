        ...
        return Object.assign({}, vendorMixins, {
            ...
            _gatherStepData: function (step) {
                switch (step) {
                    ...
                    case 3: {
                        var elements = [];
                        var byLayer = (this.architectureResult || {}).elements_by_layer || {};
            ...
            importBriefFromDoc: function (event) {
                ...
                fetch(API_BASE + '/' + self.solutionId + '/extract-brief', {
                    ...
                    if (body.success && body.data && body.data.brief) {
                        self.problemStatement = body.data.brief;
                        if (body.data.ingestion_started) {
                            var docLabel = files.length > 1 ? 'these documents' : 'this document';
                            self.briefIngestionNotice = 'Architecture elements are being extracted from ' + docLabel + ' in the background — review proposals on the Blueprint page once you reach it.';
                        }
                    } else {
                        alert((body.error || body.message) || 'Could not extract text from the uploaded document(s).');
                    }
