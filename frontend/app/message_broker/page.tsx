...
export default function MessageBrokerListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["message_broker", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: MessageBroker[]; total: number } | MessageBroker[]>("/api/message_brokers", {
  ...
  const items: MessageBroker[] = Array.isArray(data) ? data : ((data as { items: MessageBroker[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">MessageBrokers</h1>
          <p className="text-sm text-muted-foreground">
            Manage messagebroker records
          </p>
        </div>
        <Link href="/message_broker/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search messagebrokers..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No messagebrokers yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first messagebroker to get started.</p>
              <Link href="/message_broker/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/message_broker/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/message_broker/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/message_broker/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
