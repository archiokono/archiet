...
export default function ReportingServiceDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["reporting_service", id],
    queryFn: () => api.get<ReportingService>(`/api/reporting_services/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/reporting_services/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reporting_service"] })
      toast.success("ReportingService deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">ReportingService not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">ReportingService</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/reporting_service/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete ReportingService?</DialogTitle>
      ...
      <Card>
        ...
        <CardContent>
          <dl className="divide-y">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Title</dt>
              <dd className="flex-1 text-sm">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Document Type</dt>
              <dd className="flex-1 text-sm">

                {item.document_type ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Effective Date</dt>
              <dd className="flex-1 text-sm">

                {formatDateTime(item.effective_date)}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Expiry Date</dt>
              <dd className="flex-1 text-sm">
