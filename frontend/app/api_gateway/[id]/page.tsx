...
export default function ApiGatewayDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["api_gateway", id],
    queryFn: () => api.get<ApiGateway>(`/api/api_gateways/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/api_gateways/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["api_gateway"] })
      toast.success("ApiGateway deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">ApiGateway not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">ApiGateway</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/api_gateway/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete ApiGateway?</DialogTitle>
