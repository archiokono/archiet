...
export default function ExportServiceListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["export_service", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: ExportService[]; total: number } | ExportService[]>("/api/export_services", {
  ...
  const items: ExportService[] = Array.isArray(data) ? data : ((data as { items: ExportService[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">ExportServices</h1>
          <p className="text-sm text-muted-foreground">
            Manage exportservice records
          </p>
        </div>
        <Link href="/export_service/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search exportservices..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No exportservices yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first exportservice to get started.</p>
              <Link href="/export_service/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/export_service/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/export_service/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/export_service/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
