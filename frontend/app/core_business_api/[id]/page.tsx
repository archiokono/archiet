...
export default function CoreBusinessApiDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["core_business_api", id],
    queryFn: () => api.get<CoreBusinessApi>(`/api/core_business_apis/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/core_business_apis/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["core_business_api"] })
      toast.success("CoreBusinessApi deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">CoreBusinessApi not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">CoreBusinessApi</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/core_business_api/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete CoreBusinessApi?</DialogTitle>
