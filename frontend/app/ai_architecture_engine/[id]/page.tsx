...
export default function AiArchitectureEngineDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_architecture_engine", id],
    queryFn: () => api.get<AiArchitectureEngine>(`/api/ai_architecture_engines/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/ai_architecture_engines/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ai_architecture_engine"] })
      toast.success("AiArchitectureEngine deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">AiArchitectureEngine not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">AiArchitectureEngine</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/ai_architecture_engine/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete AiArchitectureEngine?</DialogTitle>
