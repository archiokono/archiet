...
export default function ReportingServiceListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["reporting_service", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: ReportingService[]; total: number } | ReportingService[]>("/api/reporting_services", {
  ...
  const items: ReportingService[] = Array.isArray(data) ? data : ((data as { items: ReportingService[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">ReportingServices</h1>
          <p className="text-sm text-muted-foreground">
            Manage reportingservice records
          </p>
        </div>
        <Link href="/reporting_service/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search reportingservices..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No reportingservices yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first reportingservice to get started.</p>
              <Link href="/reporting_service/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/reporting_service/${item.id}`)}
                  >
                    ...
                    <TableCell className="text-muted-foreground">{formatDate(item.effective_date)}</TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/reporting_service/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/reporting_service/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
