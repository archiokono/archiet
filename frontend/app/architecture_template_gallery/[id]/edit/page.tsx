...
export default function ArchitectureTemplateGalleryEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/architecture_template_galleries/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/architecture_template_galleries/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["architecture_template_gallery", id] })
      toast.success("ArchitectureTemplateGallery updated")
      router.push(`/architecture_template_gallery/${id}`)
    },
    onError: (err: unknown) => {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/architecture_template_gallery/${id}`}>
          <Button variant="ghost" size="icon">
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <h1 className="text-2xl font-semibold tracking-tight">
          Edit ArchitectureTemplateGallery
        </h1>
      </div>
