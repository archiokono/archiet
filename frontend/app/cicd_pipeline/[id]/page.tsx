...
export default function CicdPipelineDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["cicd_pipeline", id],
    queryFn: () => api.get<CicdPipeline>(`/api/cicd_pipelines/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/cicd_pipelines/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cicd_pipeline"] })
      toast.success("CicdPipeline deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">CicdPipeline not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">CicdPipeline</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/cicd_pipeline/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete CicdPipeline?</DialogTitle>
