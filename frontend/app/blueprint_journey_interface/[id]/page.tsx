...
export default function BlueprintJourneyInterfaceDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["blueprint_journey_interface", id],
    queryFn: () => api.get<BlueprintJourneyInterface>(`/api/blueprint_journey_interfaces/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/blueprint_journey_interfaces/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["blueprint_journey_interface"] })
      toast.success("BlueprintJourneyInterface deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">BlueprintJourneyInterface not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">BlueprintJourneyInterface</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/blueprint_journey_interface/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete BlueprintJourneyInterface?</DialogTitle>
