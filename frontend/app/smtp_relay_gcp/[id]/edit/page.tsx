...
export default function SmtpRelayGcpEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/smtp_relay_gcps/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/smtp_relay_gcps/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["smtp_relay_gcp", id] })
      toast.success("SmtpRelayGcp updated")
      router.push(`/smtp_relay_gcp/${id}`)
    },
    onError: (err: unknown) => {
