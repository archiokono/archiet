...
export default function ExportServiceDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["export_service", id],
    queryFn: () => api.get<ExportService>(`/api/export_services/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/export_services/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["export_service"] })
      toast.success("ExportService deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">ExportService not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">ExportService</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/export_service/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete ExportService?</DialogTitle>
