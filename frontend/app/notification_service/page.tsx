...
export default function NotificationServiceListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["notification_service", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: NotificationService[]; total: number } | NotificationService[]>("/api/notification_services", {
  ...
  const items: NotificationService[] = Array.isArray(data) ? data : ((data as { items: NotificationService[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">NotificationServices</h1>
          <p className="text-sm text-muted-foreground">
            Manage notificationservice records
          </p>
        </div>
        <Link href="/notification_service/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search notificationservices..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No notificationservices yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first notificationservice to get started.</p>
              <Link href="/notification_service/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/notification_service/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/notification_service/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/notification_service/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
