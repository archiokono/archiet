...
export default function FrontendApplicationDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["frontend_application", id],
    queryFn: () => api.get<FrontendApplication>(`/api/frontend_applications/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/frontend_applications/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["frontend_application"] })
      toast.success("FrontendApplication deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">FrontendApplication not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">FrontendApplication</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/frontend_application/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete FrontendApplication?</DialogTitle>
