...
export default function MonitoringPlatformDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["monitoring_platform", id],
    queryFn: () => api.get<MonitoringPlatform>(`/api/monitoring_platforms/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/monitoring_platforms/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["monitoring_platform"] })
      toast.success("MonitoringPlatform deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">MonitoringPlatform not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">MonitoringPlatform</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/monitoring_platform/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete MonitoringPlatform?</DialogTitle>
