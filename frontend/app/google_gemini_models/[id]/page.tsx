...
export default function GoogleGeminiModelsDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["google_gemini_models", id],
    queryFn: () => api.get<GoogleGeminiModels>(`/api/google_gemini_models/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/google_gemini_models/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["google_gemini_models"] })
      toast.success("GoogleGeminiModels deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GoogleGeminiModels not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GoogleGeminiModels</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/google_gemini_models/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GoogleGeminiModels?</DialogTitle>
