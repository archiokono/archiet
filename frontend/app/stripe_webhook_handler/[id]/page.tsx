...
export default function StripeWebhookHandlerDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["stripe_webhook_handler", id],
    queryFn: () => api.get<StripeWebhookHandler>(`/api/stripe_webhook_handlers/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/stripe_webhook_handlers/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["stripe_webhook_handler"] })
      toast.success("StripeWebhookHandler deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">StripeWebhookHandler not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">StripeWebhookHandler</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/stripe_webhook_handler/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete StripeWebhookHandler?</DialogTitle>
