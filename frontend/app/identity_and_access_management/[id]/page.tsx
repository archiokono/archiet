...
export default function IdentityAndAccessManagementDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["identity_and_access_management", id],
    queryFn: () => api.get<IdentityAndAccessManagement>(`/api/identity_and_access_managements/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/identity_and_access_managements/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["identity_and_access_management"] })
      toast.success("IdentityAndAccessManagement deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">IdentityAndAccessManagement not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">IdentityAndAccessManagement</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/identity_and_access_management/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete IdentityAndAccessManagement?</DialogTitle>
