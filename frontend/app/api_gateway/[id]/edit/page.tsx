...
export default function ApiGatewayEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/api_gateways/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/api_gateways/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["api_gateway", id] })
      toast.success("ApiGateway updated")
      router.push(`/api_gateway/${id}`)
    },
    onError: (err: unknown) => {
