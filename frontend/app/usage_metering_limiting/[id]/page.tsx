...
export default function UsageMeteringLimitingDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["usage_metering_limiting", id],
    queryFn: () => api.get<UsageMeteringLimiting>(`/api/usage_metering_limitings/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/usage_metering_limitings/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["usage_metering_limiting"] })
      toast.success("UsageMeteringLimiting deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">UsageMeteringLimiting not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">UsageMeteringLimiting</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/usage_metering_limiting/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete UsageMeteringLimiting?</DialogTitle>
