...
export default function MonitoringPlatformListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["monitoring_platform", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: MonitoringPlatform[]; total: number } | MonitoringPlatform[]>("/api/monitoring_platforms", {
  ...
  const items: MonitoringPlatform[] = Array.isArray(data) ? data : ((data as { items: MonitoringPlatform[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">MonitoringPlatforms</h1>
          <p className="text-sm text-muted-foreground">
            Manage monitoringplatform records
          </p>
        </div>
        <Link href="/monitoring_platform/new">
          <Button>
            <Plus className="mr-2 h-4 w-4" />
            New MonitoringPlatform
          </Button>
        </Link>
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search monitoringplatforms..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No monitoringplatforms yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first monitoringplatform to get started.</p>
              <Link href="/monitoring_platform/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create MonitoringPlatform
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/monitoring_platform/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/monitoring_platform/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/monitoring_platform/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
