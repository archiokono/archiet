...
export default function GcpCohesityForSystemsBackupDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["gcp_cohesity_for_systems_backup", id],
    queryFn: () => api.get<GcpCohesityForSystemsBackup>(`/api/gcp_cohesity_for_systems_backups/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/gcp_cohesity_for_systems_backups/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gcp_cohesity_for_systems_backup"] })
      toast.success("GcpCohesityForSystemsBackup deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GcpCohesityForSystemsBackup not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GcpCohesityForSystemsBackup</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/gcp_cohesity_for_systems_backup/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GcpCohesityForSystemsBackup?</DialogTitle>
