...
export default function CodeGenerationServiceListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["code_generation_service", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: CodeGenerationService[]; total: number } | CodeGenerationService[]>("/api/code_generation_services", {
  ...
  const items: CodeGenerationService[] = Array.isArray(data) ? data : ((data as { items: CodeGenerationService[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">CodeGenerationServices</h1>
          <p className="text-sm text-muted-foreground">
            Manage codegenerationservice records
          </p>
        </div>
        <Link href="/code_generation_service/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search codegenerationservices..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No codegenerationservices yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first codegenerationservice to get started.</p>
              <Link href="/code_generation_service/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/code_generation_service/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/code_generation_service/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/code_generation_service/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
