...
export default function JourneySessionOrchestratorDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["journey_session_orchestrator", id],
    queryFn: () => api.get<JourneySessionOrchestrator>(`/api/journey_session_orchestrators/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/journey_session_orchestrators/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["journey_session_orchestrator"] })
      toast.success("JourneySessionOrchestrator deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">JourneySessionOrchestrator not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">JourneySessionOrchestrator</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/journey_session_orchestrator/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete JourneySessionOrchestrator?</DialogTitle>
