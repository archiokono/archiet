...
export default function MessageBrokerEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/message_brokers/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/message_brokers/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["message_broker", id] })
      toast.success("MessageBroker updated")
      router.push(`/message_broker/${id}`)
    },
    onError: (err: unknown) => {
