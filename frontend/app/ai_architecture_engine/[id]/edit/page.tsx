...
export default function AiArchitectureEngineEditPage({ params }: { params: { id: string } }) {
  const { id } = params
  ...
  const { data: item, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Record<string, any>>(`/api/ai_architecture_engines/${id}`),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return api.put(`/api/ai_architecture_engines/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ai_architecture_engine", id] })
      toast.success("AiArchitectureEngine updated")
      router.push(`/ai_architecture_engine/${id}`)
    },
    onError: (err: unknown) => {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/ai_architecture_engine/${id}`}>
          <Button variant="ghost" size="icon">
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <h1 className="text-2xl font-semibold tracking-tight">
          Edit AiArchitectureEngine
        </h1>
      </div>
