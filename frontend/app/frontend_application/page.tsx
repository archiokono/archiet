...
export default function FrontendApplicationListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["frontend_application", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: FrontendApplication[]; total: number } | FrontendApplication[]>("/api/frontend_applications", {
  ...
  const items: FrontendApplication[] = Array.isArray(data) ? data : ((data as { items: FrontendApplication[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">FrontendApplications</h1>
          <p className="text-sm text-muted-foreground">
            Manage frontendapplication records
          </p>
        </div>
        <Link href="/frontend_application/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search frontendapplications..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No frontendapplications yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first frontendapplication to get started.</p>
              <Link href="/frontend_application/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create FrontendApplication
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/frontend_application/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/frontend_application/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/frontend_application/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
