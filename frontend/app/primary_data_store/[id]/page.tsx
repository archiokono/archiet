...
export default function PrimaryDataStoreDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["primary_data_store", id],
    queryFn: () => api.get<PrimaryDataStore>(`/api/primary_data_stores/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/primary_data_stores/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["primary_data_store"] })
      toast.success("PrimaryDataStore deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">PrimaryDataStore not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">PrimaryDataStore</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/primary_data_store/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete PrimaryDataStore?</DialogTitle>
      ...
      <Card>
        ...
        <CardContent>
          <dl className="divide-y">
            ...
            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Storage Type</dt>
              <dd className="flex-1 text-sm">

                {item.storage_type ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Capacity</dt>
              <dd className="flex-1 text-sm">

                {item.capacity ?? <span className="text-muted-foreground">—</span>}

              </dd>
            </div>

            <div className="flex py-3">
              <dt className="w-1/3 text-sm font-medium text-muted-foreground">Connection String</dt>
              <dd className="flex-1 text-sm">

                {item.connection_string ?? <span className="text-muted-foreground">—</span>}
