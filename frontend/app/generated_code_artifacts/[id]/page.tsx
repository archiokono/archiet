...
export default function GeneratedCodeArtifactsDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["generated_code_artifacts", id],
    queryFn: () => api.get<GeneratedCodeArtifacts>(`/api/generated_code_artifacts/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/generated_code_artifacts/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["generated_code_artifacts"] })
      toast.success("GeneratedCodeArtifacts deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">GeneratedCodeArtifacts not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">GeneratedCodeArtifacts</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/generated_code_artifacts/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete GeneratedCodeArtifacts?</DialogTitle>
