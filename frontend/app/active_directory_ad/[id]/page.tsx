...
export default function ActiveDirectoryAdDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["active_directory_ad", id],
    queryFn: () => api.get<ActiveDirectoryAd>(`/api/active_directory_ads/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/active_directory_ads/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["active_directory_ad"] })
      toast.success("ActiveDirectoryAd deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">ActiveDirectoryAd not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">ActiveDirectoryAd</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/active_directory_ad/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete ActiveDirectoryAd?</DialogTitle>
