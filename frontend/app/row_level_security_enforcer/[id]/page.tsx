...
export default function RowLevelSecurityEnforcerDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["row_level_security_enforcer", id],
    queryFn: () => api.get<RowLevelSecurityEnforcer>(`/api/row_level_security_enforcers/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/row_level_security_enforcers/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["row_level_security_enforcer"] })
      toast.success("RowLevelSecurityEnforcer deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">RowLevelSecurityEnforcer not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">RowLevelSecurityEnforcer</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/row_level_security_enforcer/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete RowLevelSecurityEnforcer?</DialogTitle>
