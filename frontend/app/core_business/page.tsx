...
export default function CoreBusinessListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["core_business", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: CoreBusiness[]; total: number } | CoreBusiness[]>("/api/core_businesses", {
  ...
  const items: CoreBusiness[] = Array.isArray(data) ? data : ((data as { items: CoreBusiness[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">CoreBusinesss</h1>
          <p className="text-sm text-muted-foreground">
            Manage corebusiness records
          </p>
        </div>
        <Link href="/core_business/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search corebusinesss..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No corebusinesss yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first corebusiness to get started.</p>
              <Link href="/core_business/new">
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/core_business/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/core_business/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/core_business/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
