...
export default function CicdToolchainDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["cicd_toolchain", id],
    queryFn: () => api.get<CicdToolchain>(`/api/cicd_toolchains/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/cicd_toolchains/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cicd_toolchain"] })
      toast.success("CicdToolchain deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">CicdToolchain not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">CicdToolchain</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/cicd_toolchain/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete CicdToolchain?</DialogTitle>
