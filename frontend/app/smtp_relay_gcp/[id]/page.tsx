...
export default function SmtpRelayGcpDetailPage({ params }: { params: { id: string } }) {
  const { id } = params
  const queryClient = useQueryClient()

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["smtp_relay_gcp", id],
    queryFn: () => api.get<SmtpRelayGcp>(`/api/smtp_relay_gcps/${id}`),
  })

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/api/smtp_relay_gcps/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["smtp_relay_gcp"] })
      toast.success("SmtpRelayGcp deleted")
      window.history.back()
    },
  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-destructive">SmtpRelayGcp not found</p>
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">SmtpRelayGcp</h1>
            <p className="text-sm text-muted-foreground">ID: {item.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/smtp_relay_gcp/${id}/edit`}>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Delete SmtpRelayGcp?</DialogTitle>
