...
export default function CoreBusinessApiListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["core_business_api", debouncedSearch, sortField, sortDir, page],
    queryFn: () => api.get<{ items: CoreBusinessApi[]; total: number } | CoreBusinessApi[]>("/api/core_business_apis", {
  ...
  const items: CoreBusinessApi[] = Array.isArray(data) ? data : ((data as { items: CoreBusinessApi[] } | undefined)?.items ?? [])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">CoreBusinessApis</h1>
          <p className="text-sm text-muted-foreground">
            Manage corebusinessapi records
          </p>
        </div>
        <Link href="/core_business_api/new">
          <Button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search corebusinessapis..."
            value={search}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-sm font-medium">No corebusinessapis yet</h3>
              <p className="mt-1 text-xs text-muted-foreground">Create your first corebusinessapi to get started.</p>
              <Link href="/core_business_api/new">
                <Button variant="outline" size="sm" className="mt-4">
                  <Plus className="mr-2 h-3 w-3" />
                  Create CoreBusinessApi
                </Button>
              </Link>
            </div>
          ) : (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/core_business_api/${item.id}`)}
                  >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/core_business_api/${item.id}`}>View details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/core_business_api/${item.id}/edit`}>Edit</Link>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
